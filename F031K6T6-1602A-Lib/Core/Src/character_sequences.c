...
void Play_Celebration_Lyrics(UART_HandleTypeDef *huart) {
   ...
   while (celebration_characters[index] != '\0') {
        ...
        char strout[100];
        snprintf(strout, sizeof(strout), "Sending %.*s\r\n", word_len, celebration_characters - word_len + index);
        HAL_UART_Transmit(huart, (uint8_t *)strout, strlen(strout), 1000);
...
void Play_Debug_Sequence(UART_HandleTypeDef *huart) {
    ...
    while (debug_characters[index] != '\0') {
        ...
        char strout[100];
        snprintf(strout, sizeof(strout), "Sending %.*s\r\n", word_len, debug_characters - word_len + index);
        HAL_UART_Transmit(huart, (uint8_t *)strout, strlen(strout), 1000);
    }
}
