...
max_ADC_values = 0 # When there are >5 max ADC values (4095), we stop
...
while True:
    ...
    labeled_str_out = "Input Voltage: " + str(voltage) + ", DMM Voltage: " + str(dmm_voltage) + ", Raw ADC Value: " + str(raw_adc_value) + ", Iteration Time: " + str(elapsed_time)
    ...
    if (raw_adc_value >= 4095): max_ADC_values += 1
    if (max_ADC_values >= 5): break
