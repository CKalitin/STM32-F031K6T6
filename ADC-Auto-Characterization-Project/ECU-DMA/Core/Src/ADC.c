...
void Get_Averaged_ADC_Values(int* adcValuesAveraged, int* adcValuesADCAdjusted, int* adcValuesCurrentAdjusted){
  *adcValuesAveraged = adcAveragedValue; // Set the pointer output
  ...
  int adcError = -61.1 + (0.0116 * (*adcValuesAveraged)); // This is a predetermined error polynomial
  int currentSensorADCError = 51.1 + (-0.0239 * (*adcValuesAveraged)); // This is the predetermined erroy polynomial for the current sensor
